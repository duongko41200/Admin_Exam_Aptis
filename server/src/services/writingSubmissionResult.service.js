...
export class WritingResultService {
  ...
  static async createResult(resultData) {
    try {
      // Validate required fields
      if (!resultData.writingId || !resultData.userId || !resultData.content) {
        throw new BadRequestError(
          "Missing required fields: writingId, userId, content"
        );
      }

      // Check if result already exists
      const existingResult = await WritingResultRepository.findByWritingId(
        resultData.writingId
      );
      if (existingResult) {
        throw new ConflcRequestError(
          `Result with writingId ${resultData.writingId} already exists`
        );
      }

      // Validate score structure
      if (resultData.score) {
        this.validateScoreStructure(resultData.score);
      }
